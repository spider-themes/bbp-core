...
class bbpc_grid_thanks extends d4p_grid {
	public $_sanitize_orderby_fields = [ 'a.action_id', 'a.post_id', 'user_from', 'user_to' ];
	public $_table_class_name        = 'bbpc-grid-thanks';
	...
	private function _self( $args, $getback = false ) {
		$url = 'admin.php?page=bbp-core-thanks-list&' . $args;

		if ( $getback ) {
			$url .= '&bbpc_handler=getback';
			$url .= '&_wpnonce=' . wp_create_nonce( 'bbp-core-thanks' );
	...
	protected function extra_tablenav( $which ) {
		if ( $which == 'top' ) {
			...
			submit_button( __( 'Filter', 'bbp-core' ), 'button', false, false, [ 'id' => 'bbpc-thanks-submit' ] );
			echo '</div>';
		}
	}
	...
	public function rows_per_page() {
		...
		$per_page = get_user_meta( $user, 'bbpc_rows_thanks_per_page', true );
	...
	public function prepare_items() {
		...
		$sql = [
			...
			'from'   => [
				bbpc_db()->actions . ' a',
				'INNER JOIN ' . bbpc_db()->wpdb()->posts . ' p ON p.ID = a.post_id AND p.post_type IN (' . $_sel_type . ')',
		...
		$query = bbpc_db()->build_query( $sql );

		$this->items = bbpc_db()->run( $query );

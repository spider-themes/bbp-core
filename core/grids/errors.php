...
class bbpc_grid_errors extends d4p_grid {
	public $_sanitize_orderby_fields = [ 'm.meta_id', 'p.post_title', 'p.post_author', 'p.post_parent' ];
	public $_checkbox_field          = 'meta_id';
	public $_table_class_name        = 'bbpc-grid-errors';
	...
	public function rows_per_page() {
		...
		$per_page = get_user_meta( $user, 'bbpc_rows_errors_per_page', true );
	...
	public function column_file( $item ) {
		$actions = [
			'delete' => sprintf( '<a href="admin.php?page=bbp-core-errors&single-action=%s&error=%s&bbpc_handler=getback&_wpnonce=%s">%s</a>', 'delete', $item->meta_id, wp_create_nonce( 'bbp-core-error' ), __( 'Delete', 'bbp-core' ) ),
		];
	...
	public function column_topic( $item ) {
		...
		$actions = [
			'narrow' => sprintf( '<a href="admin.php?page=bbp-core-errors&bbp_topic_id=%s">%s</a>', $topic_id, __( 'Filter', 'bbp-core' ) ),
	...
	public function prepare_items() {
		...
		$query_where  = [ 'p.post_type in (' . $_sel_attached . ')', "m.meta_key = '_bbp_attachment_upload_error'" ];
		$query_errors = 'SELECT SQL_CALC_FOUND_ROWS m.meta_id, m.meta_value, p.* FROM ' . bbpc_db()->wpdb()->postmeta . ' m INNER JOIN ' . bbpc_db()->wpdb()->posts . ' p ON p.ID = m.post_id';
		...
		$this->items = bbpc_db()->get_results( $query_errors );

...
class bbpc_grid_users extends d4p_grid {
	public $_table_class_name = 'bbpc-grid-users';
	...
	public function __construct( $args = [] ) {
		...
		$this->bbp_roles = bbpc_get_user_roles();
	...
	protected function get_views() {
		return [
			'default'             => '<a class="' . ( $this->current_view == 'default' ? 'current' : '' ) . '" href="admin.php?page=bbp-core-users">' . __( 'All Forum Users', 'bbp-core' ) . '</a>',
			'topic_favorites'     => '<a class="' . ( $this->current_view == 'topic_favorites' ? 'current' : '' ) . '" href="admin.php?page=bbp-core-users&view=topic_favorites">' . __( 'Users with Favorite Topics', 'bbp-core' ) . '</a>',
			'topic_subscriptions' => '<a class="' . ( $this->current_view == 'topic_subscriptions' ? 'current' : '' ) . '" href="admin.php?page=bbp-core-users&view=topic_subscriptions">' . __( 'Users with Topic Subscriptions', 'bbp-core' ) . '</a>',
			'forum_subscriptions' => '<a class="' . ( $this->current_view == 'forum_subscriptions' ? 'current' : '' ) . '" href="admin.php?page=bbp-core-users&view=forum_subscriptions">' . __( 'Users with Forum Subscriptions', 'bbp-core' ) . '</a>',
		];
	}
	...
	protected function extra_tablenav( $which ) {
		if ( $which == 'top' ) {
			...
			submit_button( __( 'Filter', 'bbp-core' ), 'button', false, false, [ 'id' => 'bbpc-users-submit' ] );
			echo '</div>';
		}
	}
	...
	public function rows_per_page() {
		...
		$per_page = get_user_meta( $user, 'bbpc_rows_users_per_page', true );
	...
	protected function get_sortable_columns() {
		return [
			...
			'topics'   => [ 'usr.topics', false ],
			'replies'  => [ 'usr.replies', false ],
		];
	}
	...
	public function column_sig( $item ) {
		...
		if ( $sig != '' ) {
			...
			$render = '<div class="bbpc-signature">' . $sig . '</div>';
		}
	...
	public function column_date( $item ) {
		if ( $item->{bbpc_plugin()->user_meta_key_last_activity()} != '' ) {
			$time = intval( $item->{bbpc_plugin()->user_meta_key_last_activity()} ) + d4p_gmt_offset() * 3600;

			return date( 'Y.m.d', $time ) . '<br/>@ ' . date( 'H:i:s', $time );
		} elseif ( $item->bbp_last_activity != '' ) {
	...
	public function prepare_items() {
		...
		add_action( 'pre_user_query', [ $this, 'users_query' ] );
	...
	public function users_query( $query ) {
		...
		$join               = 'SELECT DISTINCT post_author AS ID FROM ' . bbpc_db()->wpdb()->posts . " WHERE post_type in ('" . join( "', '", $post_types ) . "')  UNION SELECT DISTINCT CAST(m.meta_value AS UNSIGNED) AS ID FROM " . bbpc_db()->wpdb()->postmeta . ' m INNER JOIN ' . bbpc_db()->wpdb()->posts . " p ON p.ID = m.post_id WHERE m.meta_key IN ('_bbp_subscription', '_bbp_favorite') AND post_type IN ('" . bbp_get_forum_post_type() . "', '" . bbp_get_topic_post_type() . "')";
		$query->query_from .= ' INNER JOIN (' . $join . ') bbpc_valid ON bbpc_valid.ID = ' . bbpc_db()->wpdb()->users . '.ID ';
		...
		if ( $this->current_view != 'default' ) {
			if ( $this->current_view == 'forum_subscriptions' ) {
				...
				if ( $_sel_forum == 0 ) {
					$query->query_where .= ' AND (' . bbpc_db()->wpdb()->users . '.ID IN (SELECT DISTINCT CAST(m.meta_value AS UNSIGNED) AS user_id FROM ' . bbpc_db()->wpdb()->postmeta . ' m INNER JOIN ' . bbpc_db()->wpdb()->posts . " p ON p.ID = m.post_id  WHERE m.meta_key = '_bbp_subscription' AND p.post_type IN ('" . bbp_get_forum_post_type() . "')))";
				} else {
					$query->query_where .= ' AND (' . bbpc_db()->wpdb()->users . '.ID IN (SELECT DISTINCT CAST(m.meta_value AS UNSIGNED) AS user_id FROM ' . bbpc_db()->wpdb()->postmeta . " m WHERE m.meta_key = '_bbp_subscription' AND m.post_id = " . $_sel_forum . '))';
				}
			} elseif ( $this->current_view == 'topic_subscriptions' || $this->current_view == 'topic_favorites' ) {
				$_key       = $this->current_view == 'topic_subscriptions' ? '_bbp_subscription' : '_bbp_favorite';
				$_sel_topic = isset( $_GET['filter-topic'] ) && ! empty( $_GET['filter-topic'] ) ? absint( $_GET['filter-topic'] ) : 0;

				if ( $_sel_topic == 0 ) {
					$query->query_where .= ' AND (' . bbpc_db()->wpdb()->users . '.ID IN (SELECT DISTINCT CAST(m.meta_value AS UNSIGNED) AS user_id FROM ' . bbpc_db()->wpdb()->postmeta . ' m INNER JOIN ' . bbpc_db()->wpdb()->posts . " p ON p.ID = m.post_id  WHERE m.meta_key = '" . $_key . "' AND p.post_type IN ('" . bbp_get_topic_post_type() . "')))";
				} else {
					$query->query_where .= ' AND (' . bbpc_db()->wpdb()->users . '.ID IN (SELECT DISTINCT CAST(m.meta_value AS UNSIGNED) AS user_id FROM ' . bbpc_db()->wpdb()->postmeta . " m WHERE m.meta_key = '" . $_key . "' AND m.post_id = " . $_sel_topic . '))';
				}
			}
		}
		...
		if ( $query->query_vars['orderby'] == 'usr.replies' || $query->query_vars['orderby'] == 'usr.topics' ) {
			...
			} elseif ( $query->query_vars['orderby'] == 'usr.topics' ) {
	...
	private function calculate_counts() {
		...
		if ( ! empty( $users ) ) {
			$sql = 'SELECT post_type, post_author, count(*) AS counter FROM ' . bbpc_db()->wpdb()->posts . " WHERE post_type IN ('" . bbp_get_reply_post_type() . "', '" . bbp_get_topic_post_type() . "') AND post_status IN ('pending', 'publish', 'closed') AND post_author IN (" . join( ', ', $users ) . ') GROUP BY post_type, post_author';
			$raw = bbpc_db()->get_results( $sql );

			foreach ( $raw as $row ) {
				if ( ! isset( $this->items[ $row->post_author ]->data->forums ) ) {
					$this->items[ $row->post_author ]->data->forums = [];
				}
	...
	private function calculate_engagements() {
		...
		if ( ! empty( $users ) ) {
			$sql = 'SELECT CAST(m.meta_value AS UNSIGNED) AS user_id, SUBSTR(m.meta_key, 6) AS type, p.post_type, COUNT(*) AS counter 
                    FROM ' . bbpc_db()->wpdb()->postmeta . ' m INNER JOIN ' . bbpc_db()->wpdb()->posts . " p ON p.ID = m.post_id 
			...
			foreach ( $raw as $row ) {
				if ( ! isset( $this->items[ $row->user_id ]->data->engagements ) ) {
					$this->items[ $row->user_id ]->data->engagements = [
						'topic_subscriptions' => 0,
						'forum_subscriptions' => 0,
						'topic_favorites'     => 0,
					];

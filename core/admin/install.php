...
function bbpc_list_database_tables() {
	global $wpdb;

	return [
		$wpdb->prefix . 'bbpc_actionmeta'  => 4,
		$wpdb->prefix . 'bbpc_actions'     => 6,
		$wpdb->prefix . 'bbpc_attachments' => 2,
		$wpdb->prefix . 'bbpc_online'      => 10,
		$wpdb->prefix . 'bbpc_tracker'     => 5,
	];
}

function bbpc_install_database() {
	global $wpdb;
	...
	$tables = [
		'actionmeta'  => $wpdb->prefix . 'bbpc_actionmeta',
		'actions'     => $wpdb->prefix . 'bbpc_actions',
		'attachments' => $wpdb->prefix . 'bbpc_attachments',
		'online'      => $wpdb->prefix . 'bbpc_online',
		'tracker'     => $wpdb->prefix . 'bbpc_tracker',
	];
...
function bbpc_check_database() {
	global $wpdb;

	$result = [];
	$tables = bbpc_list_database_tables();
	...
	foreach ( $tables as $table => $count ) {
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$table'" ) == $table ) {
			...
			if ( $count != count( $columns ) ) {
				$result[ $table ] = [
					'status' => 'error',
					'msg'    => __( 'Some columns are missing.', 'bbp-core' ),
				];
			} else {
				$result[ $table ] = [ 'status' => 'ok' ];
			}
		} else {
			$result[ $table ] = [
				'status' => 'error',
				'msg'    => __( 'Table missing.', 'bbp-core' ),
			];
		}
	}
...
function bbpc_convert_attachments_assignments() {
	global $wpdb;

	$sql = 'INSERT IGNORE INTO ' . $wpdb->prefix . 'bbpc_attachments 
...
function bbpc_convert_forum_settings() {
	global $wpdb;

	$sql = 'select * from ' . $wpdb->postmeta . " where meta_key in ('_gdbbatt_settings', '_bbpc_privacy_settings') order by post_id";
	...
	foreach ( $posts as $post_id => $data ) {
		$settings = bbpc_default_forum_settings();

		foreach ( $data as $meta_key => $obj ) {
			if ( is_array( $obj ) && ! empty( $obj ) ) {
				if ( $meta_key == '_bbpc_privacy_settings' ) {
		...
		update_post_meta( $post_id, '_bbpc_settings', $settings );
		delete_post_meta( $post_id, '_gdbbatt_settings' );
		delete_post_meta( $post_id, '_bbpc_privacy_settings' );
...
function bbpc_forum_last_post_date() {
	global $wpdb;
	...
	if ( ! empty( $forums ) ) {
		...
		foreach ( $forums as $forum_id ) {
			if ( isset( $list[ $forum_id ] ) ) {
				...
				$children = bbpc_get_forum_children_ids( $forum_id );
...
function bbpc_truncate_database_tables() {
	global $wpdb;

	$tables = array_keys( bbpc_list_database_tables() );
...
function bbpc_drop_database_tables() {
	global $wpdb;

...
class Template {
	public function __construct() {
		add_action( 'bbpc_template_before_topics_loop', array( $this, 'before_topics_loop' ), 10, 2 );
	...
	public function before_topics_loop( $posts, $users ) {
		if ( is_user_logged_in() && Icons::instance()->settings['forum_mark_replied'] ) {
			bbpc_cache()->userreplied_run_bulk_topics( $posts );
		}

		if ( Plugin::instance()->is_enabled( 'private-topics' ) ) {
			$run = bbp_is_single_forum() ? bbpc_private_topics()->is_enabled_topic_private() : count( $posts ) > 1;

			if ( $run ) {
				bbpc_cache()->private_run_bulk_posts( $posts );
			}
		}

		if ( Plugin::instance()->is_enabled( 'private-replies' ) ) {
			bbpc_cache()->private_run_count_topic_replies( $posts );
		}
	}

	public function before_replies_loop( $posts, $users ) {
		if ( Plugin::instance()->is_enabled( 'private-replies' ) && bbpc_private_replies()->is_enabled_reply_private() ) {
			bbpc_cache()->private_run_bulk_posts( $posts );
		}
	}
}

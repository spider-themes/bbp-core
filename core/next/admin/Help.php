...
class Help {
	...
	public function help_tab_sidebar( $plugin = 'bbp-core', $title = 'BBP Core' ) {
	...
	public function help_tab_getting_help() {
		...
		$screen->add_help_tab(
			array(
				...
				'content' => '<h2>' . __( "Found a bug?", "bbp-core" ) . '</h2><p>' . __( "If you find a bug in BBP Core, you can report it in the support forum.", "bbp-core" ) .
		...
		$screen->add_help_tab(
			array(
				...
				'content' => '<h2>' . __( "Setup Wizard", "bbp-core" ) . '</h2><p>' . __( "If you need to quickly reconfigure the plugin, you can access the Setup Wizard again.", "bbp-core" ) .
				             '</p><p><a href="' . admin_url( 'admin.php?page=bbp-core-wizard' ) . '" class="button-primary" target="_blank">' . __( "Open Setup Wizard", "bbp-core" ) . '</a></p>'
			)
		);
	}

	public function help_for_features( $screen ) {
		if ( bbpc_admin()->panel == 'custom-views' ) {
			$screen->add_help_tab(
				array(
					'id'      => 'bbpc-help-features-custom-views',
		...
		} else if ( bbpc_admin()->panel == 'schedule-topic' ) {
			$screen->add_help_tab(
				array(
					'id'      => 'bbpc-help-features-schedule-topic',

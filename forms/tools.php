...
$panels = [
	...
	'removeips'  => [
		'title'       => __( "Remove Logged IP's", 'bbp-core' ),
		'icon'        => 'user-secret',
		'button'      => 'submit',
		'button_text' => __( 'Remove', 'bbp-core' ),
	...
	'bbcodes'    => [
		...
		'icon'   => 'pencil',
		'type'   => 'bbcodes',
		'break'  => __( 'List of BBCodes and Shortcodes', 'bbp-core' ),
		'button' => 'none',
	...
	'updater'    => [
		'title'       => __( 'Recheck and Update', 'bbp-core' ),
		'icon'        => 'refresh',
	...
	'cleanup'    => [
		'title'       => __( 'Data Cleanup', 'bbp-core' ),
		'icon'        => 'trash',
		'button'      => 'submit',
		'button_text' => __( 'Cleanup', 'bbp-core' ),
		'info'        => __( 'Using this tool, you can cleanup old and obsolete plugin data.', 'bbp-core' ),
	],
	'export'     => [
		'title'       => __( 'Export Settings', 'bbp-core' ),
		'icon'        => 'download',
		'button'      => 'button',
		'button_text' => __( 'Export', 'bbp-core' ),
		'link'        => admin_url( 'admin.php?page=bbp-core-tools&run=export&bbpc_handler=getback&_ajax_nonce=' . wp_create_nonce( 'dev4press-plugin-export' ) ),
		'info'        => __( 'Using this tool, you can export all plugin settings into JSON formatted file with compressed output.', 'bbp-core' ),
	],
	'import'     => [
		'title'       => __( 'Import Settings', 'bbp-core' ),
		'icon'        => 'upload',
		'button'      => 'submit',
		'button_text' => __( 'Import', 'bbp-core' ),
		'info'        => __( 'Using this tool, you can import all plugin settings from valid export file.', 'bbp-core' ),
	],
	'remove'     => [
		'title'       => __( 'Reset / Remove', 'bbp-core' ),
		'icon'        => 'remove',
		'button'      => 'submit',
		'button_text' => __( 'Remove', 'bbp-core' ),
...
<form method="post" action="" enctype="multipart/form-data">
	<?php settings_fields( 'bbp-core-tools' ); ?>
	<input type="hidden" value="<?php echo $_panel; ?>" name="bbpctools[panel]" />
		...
		<?php if ( $_panel != 'index' && $panels[ $_panel ]['button'] != 'none' ) { ?>
			<div class="d4p-panel-buttons">
				<?php if ( isset( $panels[ $_panel ]['link'] ) ) { ?>
					<a id="bbpc-tool-<?php echo $_panel; ?>" class="button-primary" href="<?php echo $panels[ $_panel ]['link']; ?>"><?php echo $panels[ $_panel ]['button_text']; ?></a>
				<?php } else { ?>
					<input id="bbpc-tool-<?php echo $_panel; ?>" class="button-primary" type="<?php echo $panels[ $_panel ]['button']; ?>" value="<?php echo $panels[ $_panel ]['button_text']; ?>" />
				<?php } ?>
			</div>
		<?php } ?>
	</div>
	<div class="d4p-content-right">
		...
		if ( $_panel == 'index' ) {
			...
			if ( isset( $panels[ $_panel ]['type'] ) && $panels[ $_panel ]['type'] == 'bbcodes' ) {
				...
				foreach ( bbpc_get_bbcodes_list() as $code => $obj ) {
					?>
			...
			} elseif ( isset( $panels[ $_panel ]['type'] ) && $panels[ $_panel ]['type'] == 'shortcodes' ) {
				require_once BBPC_PATH . 'core/functions/bbcodes.php';

				foreach ( bbpc_get_shortcodes_list() as $code => $obj ) {
					?>
						<div class="d4p-group d4p-group-bbcodeslist">
							<h3>[<?php echo $code; ?>] - <?php echo $obj['title']; ?></h3>
							<div class="d4p-group-inner">
				...
				include BBPC_PATH . 'forms/tools/' . $_panel . '.php';
				...
				if ( $_panel != 'index' && $_panel != 'export' && $panels[ $_panel ]['button'] != 'none' ) {
					...
					<div style="padding-top: 15px; border-top: 1px solid #777; max-width: 800px;">
						<input id="bbpc-tool-<?php echo $_panel; ?>-sec" class="button-primary" type="<?php echo $panels[ $_panel ]['button']; ?>" value="<?php echo $panels[ $_panel ]['button_text']; ?>" />
					</div>

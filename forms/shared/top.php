...
$pages  = bbpc_admin()->menu_items;
$_page  = bbpc_admin()->page;
...
if ( ! empty( $panels ) ) {
	...
	if ( ! in_array( $_panel, $_available ) ) {
		$_panel             = 'index';
		bbpc_admin()->panel = false;
	}
}

$_classes = [ 'd4p-wrap', 'wpv-' . BBPC_WPV, 'd4p-page-' . $_page ];
				...
				<li class="d4p-nav-button">
					...
					<ul>
						...
						foreach ( $pages as $page => $obj ) {
							$url = 'admin.php?page=bbp-core-' . $page;
				...
				<li class="d4p-nav-button">
					...
					<ul>
						...
						foreach ( $panels as $panel => $obj ) {
							if ( $panel != $_panel ) {
								echo '<li><a href="admin.php?page=bbp-core-' . $_page . '&panel=' . $panel . '"><i aria-hidden="true" class="' . ( d4p_get_icon_class( $obj['icon'], 'fw' ) ) . '"></i> ' . $obj['title'] . '</a></li>';
							} else {
								echo '<li class="d4p-nav-current"><i aria-hidden="true" class="' . ( d4p_get_icon_class( $obj['icon'], 'fw' ) ) . '"></i> ' . $obj['title'] . '</li>';
							}
						}

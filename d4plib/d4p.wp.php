...
if ( ! function_exists( 'd4p_enqueue_color_picker' ) ) {
	function d4p_enqueue_color_picker() {
		if ( is_admin() ) {
			...
			wp_enqueue_script( 'iris', admin_url( 'js/iris.min.js' ), [ 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ] );
			wp_enqueue_script( 'wp-color-picker', admin_url( "js/color-picker$suffix.js" ), [ 'iris' ] );
...
if ( ! function_exists( 'd4p_is_current_user_roles' ) ) {
	function d4p_is_current_user_roles( $roles = [] ) {
...
if ( ! function_exists( 'd4p_list_post_types' ) ) {
	function d4p_list_post_types( $args = [] ) {
		$list       = [];
...
if ( ! function_exists( 'd4p_list_taxonomies' ) ) {
	function d4p_list_taxonomies( $args = [] ) {
		$list       = [];
...
if ( ! function_exists( 'd4p_is_plugin_active' ) ) {
	function d4p_is_plugin_active( $plugin ) {
		return in_array( $plugin, (array) get_option( 'active_plugins', [] ) ) || d4p_is_plugin_active_for_network( $plugin );
	}
}
...
if ( ! function_exists( 'd4p_is_login_page' ) ) {
	function d4p_is_login_page( $action = '' ) {
		$login_page = in_array( $GLOBALS['pagenow'], [ 'wp-login.php', 'wp-register.php' ] );

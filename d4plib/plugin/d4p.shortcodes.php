	...
	abstract class d4p_shortcodes_core {
		...
		protected function _register() {
			...
			foreach ( $list as $shortcode ) {
				...
				add_shortcode( $name, [ $this, 'shortcode_' . $shortcode ] );
		...
		protected function _shortcake() {
			add_action( 'shortcode_ui_before_do_shortcode', [ $this, 'shortcake_before' ] );
		}

		protected function _args( $code ) {
			return isset( $this->shortcodes[ $code ]['args'] ) ? $this->shortcodes[ $code ]['args'] : [];
		}

		protected function _atts( $code, $atts = [] ) {
		...
		protected function _regex( $list = [] ) {
			if ( empty( $list ) ) {

...
if ( ! class_exists( 'd4p_widget_core' ) ) {
	class d4p_widget_core extends WP_Widget {
		...
		public function __construct( $id_base = false, $name = '', $widget_options = [], $control_options = [] ) {
			$widget_options  = empty( $widget_options ) ? [
				'customize_selective_refresh' => $this->selective_refresh,
				'classname'                   => 'cls_' . $this->widget_base,
				'description'                 => $this->widget_description,
			] : $widget_options;
			$control_options = empty( $control_options ) ? [] : $control_options;
		...
		private function _widget_id( $args ) {
			$this->widget_id = str_replace( [ '-', '_' ], [ '', '' ], ( $args['widget_id'] ?? $this->widget_base ) );
		}
		...
		public function simple_render( $instance = [] ) {

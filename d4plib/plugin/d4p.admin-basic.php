...
if ( ! class_exists( 'd4p_admin_core_basic' ) ) {
	abstract class d4p_admin_core_basic {
		...
		public function help_tab_sidebar() {
			$links = apply_filters(
				$this->plugin_prefix . 'admin_help_sidebar_links',
				[
		...
		protected function load_postget_back() {
			...
			} elseif ( isset( $_GET[ $this->var_handler() ] ) && $_GET[ $this->var_handler() ] == 'getback' ) {
		...
		protected function enqueue_dev4press_library() {
			...
			wp_enqueue_style( 'd4plib-fontawesome', $this->url . 'd4plib/resources/fontawesome/css/font-awesome.min.css', [], D4P_FONTAWESOME );

			wp_enqueue_style( 'd4plib-font', $this->file( 'css', 'font', true ), [], d4p_library_enqueue_ver() );
			wp_enqueue_style( 'd4plib-shared', $this->file( 'css', 'shared', true ), [], d4p_library_enqueue_ver() );
			wp_enqueue_style( 'd4plib-admin', $this->file( 'css', 'admin', true ), [ 'd4plib-shared' ], d4p_library_enqueue_ver() );

			wp_enqueue_script( 'd4plib-shared', $this->file( 'js', 'shared', true ), [ 'jquery', 'wp-color-picker' ], d4p_library_enqueue_ver(), true );
			wp_enqueue_script( 'd4plib-admin', $this->file( 'js', 'admin', true ), [ 'd4plib-shared' ], d4p_library_enqueue_ver(), true );
			...
			if ( $this->panel == 'about' ) {
				wp_enqueue_style( 'd4plib-grid', $this->file( 'css', 'grid', true ), [], d4p_library_enqueue_ver() );
			}
		}
		...
		protected function enqueue_dev4press_library_widgets() {
			...
			wp_enqueue_style( 'd4plib-widgets', $this->file( 'css', 'widgets', true ), [ 'wp-color-picker' ], d4p_library_enqueue_ver() );
			wp_enqueue_script( 'd4plib-widgets', $this->file( 'js', 'widgets', true ), [ 'jquery', 'suggest', 'wp-color-picker', 'jquery-ui-sortable' ], d4p_library_enqueue_ver(), true );
		}

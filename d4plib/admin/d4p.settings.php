...
if ( ! class_exists( 'd4pSettingElement' ) ) {
	class d4pSettingElement {
		...
		function __construct( $type, $name, $title = '', $notice = '', $input = 'text', $value = '', $source = '', $data = '', $args = [] ) {
			$this->type   = $type;
			$this->name   = $name;
			$this->title  = $title;
			$this->notice = $notice;
...
if ( ! class_exists( 'd4pSettingsRender' ) ) {
	class d4pSettingsRender {
		...
		public function render() {
			foreach ( $this->groups as $group => $obj ) {
				if ( isset( $obj['type'] ) && $obj['type'] == 'separator' ) {
					$args = isset( $obj['args'] ) ? $obj['args'] : [];

					$classes = [ 'd4p-group-separator', 'd4p-group-' . $group ];
					...
					$classes = [ 'd4p-group', 'd4p-group-' . $group ];
		...
		private function render_option( $setting, $group ) {
			...
			$call_function = [ $this, 'draw_' . $setting->input ];
		...
		private function draw_html( $element, $value, $name_base, $id_base ) {
			...
			echo sprintf(
				'<label for="%s"><span class="d4p-accessibility-show-for-sr">%s: </span></label><textarea%s name="%s" id="%s" class="widefat">%s</textarea>',
				$id_base,
				$element->title,
				$readonly,
				esc_attr( $name_base ),
		...
		private function draw_x_by_y( $element, $value, $name_base, $id_base, $cls = '' ) {
			...
			echo sprintf(
				'<label for="%s_x"><span class="d4p-accessibility-show-for-sr">%s - X: </span></label><input%s type="text" name="%s[x]" id="%s_x" value="%s" class="widefat" />',
				$id_base,
				$element->title,
				$readonly,
				esc_attr( $name_base ),
			...
			echo ' x ';
			echo sprintf(
				'<label for="%s_y"><span class="d4p-accessibility-show-for-sr">%s - Y: </span></label><input%s type="text" name="%s[y]" id="%s_y" value="%s" class="widefat" />',
				$id_base,
				$element->title,
				$readonly,
				esc_attr( $name_base ),
...
(function($){
    $(function(){
		...
		private function draw_text( $element, $value, $name_base, $id_base, $type = 'text' ) {
			...
			echo sprintf(
				'<label for="%s"><span class="d4p-accessibility-show-for-sr">%s: </span></label><input%s%s type="%s" name="%s" id="%s" value="%s" class="widefat" />',
				$id_base,
				$element->title,
				$readonly,
				$placeholder,
		...
		private function draw_number( $element, $value, $name_base, $id_base ) {
			...
			echo sprintf(
				'<label for="%s"><span class="d4p-accessibility-show-for-sr">%s: </span></label><input%s type="number" name="%s" id="%s" value="%s" class="widefat"%s%s%s />',
				$id_base,
				$element->title,
				$readonly,
				esc_attr( $name_base ),
		...
		private function draw_checkboxes( $element, $value, $name_base, $id_base, $multiple = true ) {
			...
			foreach ( $data as $key => $title ) {
				...
				echo sprintf(
					'<label><input type="%s" value="%s" name="%s%s"%s class="widefat" />%s</label>',
	...
	class d4pSettingsProcess {
		...
		public function process( $request = false ) {
			...
			foreach ( $this->settings as $setting ) {
				if ( $setting->type != '_' ) {
					$post = isset( $request[ $this->base ][ $setting->type ] ) ? $request[ $this->base ][ $setting->type ] : [];

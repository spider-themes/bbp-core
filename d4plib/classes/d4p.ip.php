...
if ( ! class_exists( 'd4p_core_ips' ) ) {
	class d4p_core_ips {
		...
		protected static $private_ipv6 = [
			'::1/128',
			'fd00::/8',
		];

		protected static $cloudflare_ipv4 = [
			'103.21.244.0/22',
			'103.22.200.0/22',
			'103.31.4.0/22',
			'104.16.0.0/12',
		...
		protected static $cloudflare_ipv6 = [
			'2400:cb00::/32',
			'2405:b500::/32',
			'2606:4700::/32',
			'2803:f800::/32',
		...
		public static function get_all_ips() {
			$ips  = [];
			$keys = [ 'HTTP_CF_CONNECTING_IP', 'HTTP_CLIENT_IP', 'HTTP_X_REAL_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR', 'SERVER_ADDR' ];
		...
		public static function get_visitor_ip( $no_local_or_protected = false ) {
			if ( self::is_cloudflare_ip() ) {
				return self::validate_ip( $_SERVER['HTTP_CF_CONNECTING_IP'], true );
			}

			$keys = [ 'HTTP_CLIENT_IP', 'HTTP_X_REAL_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR' ];
			$ip   = '';
			...
			if ( $no_local_or_protected ) {
				$ip = self::validate_ip( $ip, true );
			} else {
				if ( $ip == '::1' ) {
					$ip = '127.0.0.1';
				} elseif ( $ip != '' ) {
					$ip = self::cleanup_ip( $ip );
				}
			}

...
if ( ! function_exists( 'd4p_scan_dir' ) ) {
	function d4p_scan_dir( $path, $filter = 'files', $extensions = [], $reg_expr = '', $full_path = false ) {
		$extensions = (array) $extensions;
		$filter     = ! in_array( $filter, [ 'folders', 'files', 'all' ] ) ? 'files' : $filter;
		...
		if ( file_exists( $path ) ) {
			...
			foreach ( $files as $file ) {
				...
				if ( empty( $extensions ) || in_array( $ext, $extensions ) ) {
					if ( substr( $file, 0, 1 ) != '.' ) {
						if ( ( is_dir( $path . $file ) && ( in_array( $filter, [ 'folders', 'all' ] ) ) ) ||
							( is_file( $path . $file ) && ( in_array( $filter, [ 'files', 'all' ] ) ) ) ||
							( ( is_file( $path . $file ) || is_dir( $path . $file ) ) && ( in_array( $filter, [ 'all' ] ) ) ) ) {
								$add = $full_path ? $path : '';

							if ( $reg_expr == '' ) {
								$final[] = $add . $file;
							} elseif ( preg_match( $reg_expr, $file ) ) {
								$final[] = $add . $file;
							}
						}
					}
...
if ( ! function_exists( 'd4p_filesize_format' ) ) {
	function d4p_filesize_format( $size, $decimals = 2, $sep = ' ' ) {
		...
		if ( strlen( $_size ) <= 9 && strlen( $_size ) >= 7 ) {
			...
			$unit  = 'MB';
		} elseif ( strlen( $_size ) >= 10 ) {
			$_size = number_format( $_size / 1073741824, $decimals );
			$unit  = 'GB';
		} elseif ( strlen( $_size ) <= 6 && strlen( $_size ) >= 4 ) {
...
if ( ! function_exists( 'd4p_get_icon_class' ) ) {
	function d4p_get_icon_class( $name, $extra = [] ) {
		$class     = '';
		$d4p       = false;
		$dashicons = false;
		...
		if ( substr( $name, 0, 3 ) == 'd4p' ) {
			$class .= 'd4p-icon ' . $name;
			$d4p    = true;
		} elseif ( substr( $name, 0, 9 ) == 'dashicons' ) {
...
if ( ! function_exists( 'd4p_render_icon' ) ) {
	function d4p_render_icon( $name, $tag = 'i', $aria_hidden = true, $fw = false, $class = '', $attr = [] ) {
		$icon = '<' . $tag;
